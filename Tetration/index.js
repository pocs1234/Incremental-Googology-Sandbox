...
const Grow = dt=>{
   ...
   ,a1,a2,a3,n,n1;
   //var tickspeedLog = isFinite(v.MainNumber) ? Math.log10(v.MainNumber+1) : 308.25;
  var tickspeedLog = 0
  var tickspeedLog = Math.max(1.05^tickspeedLog, 1);
  v.SinceFGHPrestige+=dt/tickspeedLog;
   n1=v.FGH2f1.length;
   while(n1--){
...
,v = new Vue({
   ...
   ,computed:{
      Growth(){
         ...
         return Times(Times(s,Power(8, this.AchieveRowN)),(this.AlphaSeries&4)?this.ZeralumEff:1)
      }
      ...
      ,AchieveCellEff(){return this.AchieveCellN?(Math.pow(2,Math.pow(2,Math.pow(2, this.AchieveCellN-1)))):1}
      ,AchieveRowEff(){return this.AchieveRow}
      ,AutoPool:()=>[null
         ...
         while(n1--){
            ...
            for(n=this.BM0etcLengthEver[n1];n--;){
               ...
               arr[n]={
                  ...
                  ,tooltip:n?'Generate '+BM0etcText(n+n1,n1+2):'Multiply number gain'
      ...
      ,BM0etcMult_Ach(){
         ...
         //Overall bonus to all (0)...(0)[x]
         var Overall=this.AchieveCellEff*(Power(4,this.AchieveRowN))*(Achievement[0]&32?1.271828:1)*(Achievement[1]&16?1.999:1)*(Achievement[0]&128?Math.log10(v.SinceFGHPrestige+10):1);
         var BaseMult;
         while(n1--){
            switch(n1){//Bonus to (0)...(0)[n1+2] for certain base number
               case 0:
               BaseMult = (Achievement[0]&2?2:1)*(Achievement[0]&256?2:1)*(Achievement[0]&128?2:1)*(Achievement[0]&64?2:1)*(Achievement[1]&256?2:1);
               break;
               case 1:
               BaseMult = (Achievement[2]&4?2:1)*(Achievement[2]&8?2:1)*(Achievement[2]&16?2:1)*(Achievement[1]&32?2:1);
               break;
               case 2:
               BaseMult = (Achievement[2]&128?4:1)
               default:
               BaseMult = 1
            }
            arr=[];
         ...
         if(Achievement[0]&1) {arr1[0][0]=Times(arr1[0][0],4)};
         if(Achievement[0]&8) {arr1[0][0]=Times(arr1[0][0],4)};
         if(Achievement[1]&128) {arr1[0][1]=Times(arr1[0][1],4)};
         if(Achievement[0]&16) {arr1[0][2]=Times(arr1[0][2],4)};
         if(arr1[0][3]&&Achievement[1]&1) {arr1[0][3]=Times(arr1[0][3],4)};
         if(arr1[0][5]&&Achievement[1]&2) {arr1[0][5]=Times(arr1[0][5],4)};
         if(arr1[0][6]&&Achievement[1]&64) {arr1[0][5]=Times(arr1[0][6],4)};
         if(arr1[0][7]&&Achievement[1]&4) {arr1[0][7]=Times(arr1[0][7],4)};
         if(arr1[1]&&Achievement[2]&1) {arr1[1][0]=Times(arr1[1][0],4)};
         if(arr1[1]&&Achievement[2]&2) {arr1[1][3]=Times(arr1[1][3],4)};
         if(arr1[1]&&Achievement[2]&32) {arr1[1][1]=Times(arr1[1][1],4)};
      ...
      ,BM0etcMult_Bought(){
         ...
         ,Base2Incr=2+(this.Achievement[2]&256?0.2:0)
         ,Base3Incr=2+(this.Achievement[2]&64?0.2:0)
         ,Base4Incr=4+(this.Achievement[2]&256?0.4:0);
         while(n1--){
            arr=[];
            n=(bought=BM0etcBought[n1]).length;
            while(n--) arr[n]=bought[n]?Power(n1==0?Base2Incr:(n1==1?Base3Incr:(n1==2?Base4Incr:n1+2)),Plus(bought[n],-1)):1;
            arr1[n1]=arr
         }
         return arr1
      }
      ...
      ,BM0etcUnlockerEff(){
         var BM0etcLength=this.BM0etcLength,BM0etcLengthStart=this.BM0etcLengthStart,n=BM0etcLength.length-1,arr=[],Achievement=this.Achievement
         ,AchieveRowEff = (Power(2,this.AchieveRowN))*2
         ,Overall=(this.Achievement[2]&256?1:1);
         arr[n]=Overall;
      ...
      ,FGHNumberToGet(){
         ...
         return Math.floor(Times(Times(Floor(Plus(Exp(Times(x,Power(Ln(x),-3.25))),-1.5522390492173715)),(this.Challenge&255)==1?1:this.FGHChal6Eff)
         ,(Power(1.5, this.AchieveRowN))*(ach&4?1.02:1)*(ach&16?1.05:1)*(ach&64?1.1:1)*(ach&128?1.2:1)*(ach&256?1.5:1)))
      }
      ,FGH00Cant(){return this.AlphaSeries&1||LessQ(this.FGHNumber,1)}
      ,FGHbase1Cant(){return this.AlphaSeries&2||LessQ(this.FGHNumber,2)}
      ,ZeralumCant(){return this.AlphaSeries&4||LessQ(this.FGHNumber,8)}
      ,UnalumCant(){return this.AlphaSeries&8||LessQ(this.FGHNumber,16)}
      ,BalumCant(){return this.AlphaSeries&16||LessQ(this.FGHNumber,8192)}
      ...
      ,ZeralumEff(){return Plus(this.FGHNumber,1)}
   ...
   ,methods:{
      ...
      ,AlphaSeriesBuy:n=>{
         v.FGHNumber=Minus(v.FGHNumber,[1,2,8,16,8192][n]);
         v.AlphaSeries|=1<<n;
         if(n===3) v.FGHChal&=~16;
      }
   }
...
,Loop = ()=>{
   ...
   //var tickspeedLog = isFinite(v.MainNumber) ? Math.log10(v.MainNumber+1) : 308.25;
   var tickspeedLog = 0
   var dt=(Date.now()-Time.LastUpdate)*0.001*Math.max(1.05^tickspeedLog,1);
